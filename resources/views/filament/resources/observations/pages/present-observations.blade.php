@php
    ...
    $concernText = $observation ? $this->plainText($observation->concern) : '';
@endphp
        ...
        @else
            <div class="flex min-h-[32rem] flex-col">
                <div
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <span
                            class="rounded-full px-3 py-1 text-sm font-semibold ring-1 {{ $this->statusClass($observation->status) }}">
                            {{ $this->statusLabel($observation->status) }}
                        </span>

                        @if ($this->isOverdue($observation))
                            <span
                                class="rounded-full bg-red-100 px-3 py-1 text-sm font-semibold text-red-700 ring-1 ring-red-200 dark:bg-red-900/40 dark:text-red-200 dark:ring-red-700/60">
                                Overdue
                            </span>
                        @endif
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" wire:click="previousSlide" @disabled($this->slide === 0)
                        ...
                        <button type="button" wire:click="nextSlide" @disabled($this->slide >= $totalSlides - 1)
                ...
                <div class="grid flex-1 gap-0 xl:grid-cols-[minmax(0,1.15fr)_minmax(24rem,0.85fr)]">
                    <section class="border-b border-gray-200 p-4 dark:border-gray-800 xl:border-b-0 xl:border-r">
                        <div class="grid h-full gap-4 lg:grid-cols-2">
                            <div
                                class="flex min-h-72 flex-col overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                                <div
                                    class="border-b border-gray-200 px-4 py-3 text-2xl font-semibold text-gray-700 dark:border-gray-800 dark:text-gray-200">
                                    Concern Proof
                                </div>
                                <div
                                    class="grid flex-1 gap-2 p-3 {{ count($concernImages) > 1 ? 'sm:grid-cols-2' : '' }}">
                                    ...
                                    @empty
                                        <div
                                            class="flex min-h-72 items-center justify-center rounded-md bg-white text-sm text-gray-500 dark:bg-gray-950 dark:text-gray-400">
                            ...
                            <div
                                ...
                                <div
                                    class="grid flex-1 gap-2 p-3 {{ count($solvedImages) > 1 ? 'sm:grid-cols-2' : '' }}">
                    ...
                    <aside class="space-y-6 p-5">
                        <div>
                            <p
                                class="text-xl font-semibold uppercase tracking-wide text-primary-600 dark:text-primary-300">
                                {{ $observation->dealer?->name ?? 'No dealer' }} /
                                {{ $observation->pic?->department?->name ?? 'No department' }}
                            </p>
                            <h2 class="mt-3 text-5xl font-bold tracking-tight text-gray-950 dark:text-white">
                        ...
                        <div class="space-y-2">
                            <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                                Concern / Remarks</h3>
                            <p
                                class="max-h-52 overflow-y-auto whitespace-pre-line rounded-lg bg-gray-50 p-4 text-base leading-7 text-gray-800 dark:bg-gray-900 dark:text-gray-100">
                                {{ $concernText !== '' ? $concernText : 'No concern details recorded.' }}
                            </p>
                        </div>
                        ...
                        @if (filled($observation->counter_measure))
                            <div class="space-y-2">
                                <h3
                                    ...
                                    Counter Measure</h3>
                                <p
                                    class="max-h-40 overflow-y-auto whitespace-pre-line rounded-lg bg-gray-50 p-4 text-base leading-7 text-gray-800 dark:bg-gray-900 dark:text-gray-100">
